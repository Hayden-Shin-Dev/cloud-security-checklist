...
이 프로젝트는 Hayden Shin이 제작한 클라우드 보안 점검용 웹 애플리케이션입니다. 별도 데이터베이스 없이 로컬에서 실행되며, Azure, AWS, GCP, On-Premise 환경의 사전 점검, 정기 리뷰, 보안 개선 과제 도출에 사용할 수 있습니다.
...
| --- | --- |
| 제작자 | Hayden Shin |
| GitHub | [Hayden-Shin-Dev](https://github.com/Hayden-Shin-Dev) |
| 버그 제보 | shinminchuhl@gmail.com |

이 프로젝트의 원 제작자는 Hayden Shin입니다. 버그나 개선 제안은 위 이메일로 제보해 주세요.
...
이 프로젝트는 화면과 도메인 로직을 분리해 유지보수하기 쉽게 구성되어 있습니다.

- `app.py`는 Streamlit 레이아웃, 입력 위젯, 결과 표시, 다운로드 기능을 담당합니다.
- `security_checks.py`는 체크리스트 원본 데이터, 가중치 계산, 위험 등급, 권고사항, 카테고리 분석을 담당합니다.
- 점검 결과는 메모리에만 유지되며 별도 데이터베이스에 저장하지 않습니다.
- CSV 다운로드는 현재 화면 상태를 기준으로 즉시 생성됩니다.
...
각 보안 통제 항목은 다음 정보를 포함합니다.

| 필드 | 설명 |
| --- | --- |
| `id` | 항목을 식별하는 내부 키입니다. |
| `category` | 계정, 네트워크, 데이터 보호 같은 통제 영역입니다. |
| `title` | 사용자가 체크하는 보안 점검 문장입니다. |
| `weight` | 전체 점수에 반영되는 중요도 점수입니다. |
| `severity` | Critical, High, Medium 중 하나의 위험 심각도입니다. |
| `remediation_phase` | 즉시 조치, 단기 개선, 운영 개선 같은 개선 단계입니다. |
| `evidence_hint` | 점검 시 확인해야 할 증적 예시입니다. |
| `recommendation` | 미충족 시 제공되는 개선 권고사항입니다. |
...
9. 라이트 테마와 다크 테마에서 KPI 카드, 테이블, Risk Register의 대비가 유지되는지 확인합니다.
10. 좁은 브라우저 폭에서도 KPI 카드와 탭 레이아웃이 크게 깨지지 않는지 확인합니다.
...
이 대시보드는 빠른 자가 점검과 개선 우선순위 도출을 위한 도구입니다. 클라우드 API를 직접 호출해 설정을 자동 검증하지 않으며, 정식 침투 테스트나 규제 인증 심사를 대체하지 않습니다.

실제 감사나 인증 목적의 점검에서는 각 항목에 대한 증적 수집, 담당자 확인, 변경 이력 검토, 클라우드별 상세 정책 검증을 함께 수행해야 합니다.
